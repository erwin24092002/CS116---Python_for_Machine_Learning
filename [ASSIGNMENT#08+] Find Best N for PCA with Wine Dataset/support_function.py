...
def get_acc(cf_matrix): 
    count = 0
    for i in range(len(cf_matrix)): 
        for j in range(len(cf_matrix)): 
            if i==j: 
                count += cf_matrix[i][j]
    return count/np.sum(cf_matrix)
...
def addlabels(x, y, color, denta):
    for i in range(len(x)):
        plt.text(i + denta, round(y[i], 3), round(y[i], 3), ha = 'center', fontsize=6, bbox = dict(facecolor = color, alpha =.6))
